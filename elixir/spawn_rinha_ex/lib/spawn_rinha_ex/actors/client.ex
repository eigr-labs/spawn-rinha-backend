defmodule SpawnRinhaEx.Actors.Client do
  ...
  def credit(_id, _value, description)
      when not is_binary(description) or is_nil(description) or String.length(description) > 10,
  ...
  def debit(_id, _value, description)
      when not is_binary(description) or is_nil(description) or length(description) > 10,
      do: {:error, :invalid_payload}
